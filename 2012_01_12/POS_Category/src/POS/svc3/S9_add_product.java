...
public class S9_add_product {
    ...
    public static List<to_add_product> ret_products(String type, String names) {
        ...
        try {
            ...
            String s0 = "select product_name,description,price,product_qty,prod_num,is_vat,types_num,is_linient,w_commission,comm_amount"
                        + ",cat_id,cost,printing_assembly,is_active  from " + MyDB.getNames() + ".inventory2_stocks_left"
            ...
            while (rs.next()) {
                ...
                to_add_product to = new to_add_product(name, desc, price, qty, num, vat, types_num, is_linient, w_commission, comm_amount, cat_id, cost, printing_assembly, is_active);
                datas.add(to);
            }
    ...
    public static List<to_add_product> ret_products_category(String type, String names, String cat_id) {
        ...
        try {
            ...
            String s0 = "select product_name,description,price,product_qty,prod_num,is_vat,types_num,is_linient,w_commission,comm_amount,cat_id,cost,printing_assembly,is_active  from " + MyDB.getNames() + ".inventory2_stocks_left"
            ...
            while (rs.next()) {
                ...
                int printing_assembly = rs.getInt(13);
                int is_active = rs.getInt(14);
                to_add_product to = new to_add_product(name, desc, price, qty, num, vat, types_num, is_linient, w_commission, comm_amount, cat_ids, cost, printing_assembly, is_active);
                datas.add(to);
            }
    ...
    public static List<to_add_product> ret_products_search_category(String type, String names) {
        List<to_add_product> datas = new ArrayList();
        try {
            Connection conn = MyConnection.connect();
            String s0 = "select product_name,description,price,product_qty,prod_num,is_vat,types_num,is_linient,w_commission,comm_amount,cat_id,cost,printing_assembly,is_active  from " + MyDB.getNames() + ".inventory2_stocks_left"
                        + " where description like '%" + names + "%' and cat_id = '" + type + "' order by description asc";
            Statement stmt = conn.createStatement();
            ResultSet rs = stmt.executeQuery(s0);
            while (rs.next()) {
                String name = rs.getString(1);
    ...
    public static void add(String name, String description, String price, String qty, String remarks, String types, String supp_id, String vat, String assembly, List<S7_uom.to_product_uom> tt, String is_linient, String w_com, double com_amount, String cat_id, double cost, String img_path, int is_active) {
...
//            stmt.execute();
            ...
            String s1 = "insert into " + MyDB.getNames() + ".inventory2_stocks_left(prod_num,product_name,description,price,product_qty,types,types_num,is_vat,prod_assembly,is_linient,w_commission,comm_amount,cat_id,cost,img_path,is_active)values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
