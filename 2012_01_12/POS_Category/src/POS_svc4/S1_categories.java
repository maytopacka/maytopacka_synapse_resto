...
public class S1_categories {
    ...
    public static class to_name_cat {
        ...
        public to_name_cat(String name, String num, String img_path, int open_item) {
            this.name = name;
            this.num = num;
    ...
    public static List<to_name_cat> ret_categories_others(String cat_id) {
        ...
        try {
            ...
            String s0 = "select ifnull(cat_name,''),cat_num,img_path,open_item from " + MyDB.getNames() + ".category where id like '%" + cat_id + "%'";
            ...
            while (rs.next()) {
                ...
                to_name_cat to = new to_name_cat(name, num, img_path, open_item);
                datas.add(to);
            }
    ...
    public static List<to_name_cat> ret_categories_bar(String cat_id) {
        ...
        try {
            Connection conn = MyConnection.connect();
            String s0 = "select ifnull(cat_name,''),cat_num,img_path,open_item from " + MyDB.getNames() + ".category where id = '" + "10" + "'";
            Statement stmt = conn.createStatement();
            ResultSet rs = stmt.executeQuery(s0);
            while (rs.next()) {
                String name = rs.getString(1);
    ...
    public static void add_category(String name, String img_path) {
        ...
        try {
            Connection conn = MyConnection.connect();
            String s0 = "insert into " + MyDB.getNames() + ".category(cat_name,cat_num,img_path,open_item)values(?,?,?,?)";
