...
public class S1_categories {
    ...
    public static List<to_name_cat> ret_categories_others(String cat_id) {
        ...
        try {
            ...
            String s0 = "select ifnull(cat_name,''),cat_num,img_path,open_item,printing_assembly,group_id from " + MyDB.
                    getNames() + ".category where id like '%" + cat_id + "%' order by cat_name asc";
    ...
    public static List<to_name_cat> ret_categories_bar(String cat_id) {
        ...
        try {
            Connection conn = MyConnection.connect();
            String s0 = "select ifnull(cat_name,''),cat_num,img_path,open_item,printing_assembly,group_id from " + MyDB.
                    getNames() + ".category where id = '" + "10" + "' order by cat_name asc";
            Statement stmt = conn.createStatement();
            ResultSet rs = stmt.executeQuery(s0);
            while (rs.next()) {
                String name = rs.getString(1);
    ...
    public static List<to_name> ret_categories2(String names) {
        ...
        try {
            Connection conn = MyConnection.connect();
            String s0 = "select ifnull(type_name,''),type_num from " + MyDB.
                    getNames() + ".category_type where cat_name='" + names + "'";
            Statement stmt = conn.createStatement();
            ResultSet rs = stmt.executeQuery(s0);
            while (rs.next()) {
                String name = rs.getString(1);
