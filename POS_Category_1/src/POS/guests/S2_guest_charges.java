...
public class S2_guest_charges {
    ...
    public static void add_data(to_guest_charges to_guest_charges) {
        try {
            ...
            s0 = SqlStringUtil.parse(s0).setString("guest_id", to_guest_charges.guest_id).setString("guest_name", to_guest_charges.guest_name).setString("date_added", to_guest_charges.date_added).setNumber("amount", to_guest_charges.amount).setNumber("status", to_guest_charges.status).ok();
...
//                    System.out.println(my_rate + " ="+room_rate);
                    my_rate = room_rate / my_rate;
//                     System.out.println(my_rate + " ="+room_rate);
                    Date da = new Date();
                    da = DateType.datetime.parse(t.date_added);
                    int count_days = DateUtil.count_days(da, now);
//                    System.out.println(count_days + " ssss");
                    Date a = new Date();
                    a = da;
                    if (count_days > -1) {
                        for (int i = 0; i < count_days; i++) {
                            a = DateUtil.add_day(a, 1);
//                            System.out.println(DateType.sf.format(a) + " - " + t.guest_name + " + " + 1);
                            ...
                            if (exists == false) {
                                ...
                                String s2 = "insert into " + MyDB.getNames() + ".guest_charges("
                                            ...
                                            + ",:table_no"
                                            + ")";
                                s2 = SqlStringUtil.parse(s2).setString("guest_id", to_guest_charges.guest_id).setString("guest_name", to_guest_charges.guest_name).setString("date_added", to_guest_charges.date_added).setString("amount", NumberFormat.df.format(my_rate)).setNumber("status", to_guest_charges.status).setNumber("table_no", rooom_id).ok();
